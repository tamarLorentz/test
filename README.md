...
- יכולים לשנות סטטוס של בקשות (אישור, דחייה, בקשת הבהרה)
...
- ⏱️ מעקב אחרי בקשות "תקועות" (בסטטוס ממתין להבהרה מעל 15 יום)
...
- לוגיקת הרשאות מרוכזת בקובץ `utils/permissions.ts` — מקור האמת היחיד


### 7. היסטוריית הערות (Notes History)
- ✅ עובד ארגון יכול להוסיף הערה בעת שינוי סטטוס
- ✅ כל הערה נשמרת עם שם הכותב, תאריך, והסטטוס החדש
- ✅ יועץ שנכנס לעריכת בקשה רואה את כל ההערות שנכתבו על הבקשה (כולל הערות עובדי ארגון)
- ✅ מאפשר ליועץ להבין מה נדרש ממנו לפני שהוא עורך ומגיש מחדש
...
- עובדי ארגון צריכים רק לשנות סטטוס ולהוסיף הערות (View read-only + Action buttons)
- זה מונע שגיאות בעריכה ומפשט את זרימת העבודה

**יישום:** בקובץ `RequestFormPage.tsx` בדיקת תפקיד מול `ROLES` מ-`permissions.ts`. הפונקציה `getActionLabel()` מחזירה את תווית הכפתור הנכונה לפי תפקיד.
...
**יישום:** ב-`RequestFormPage.tsx` - ב-`useEffect`, קריאה לפונקציה `consultantCanEdit()` מ-`permissions.ts` שבודקת גם בעלות וגם סטטוס.
...
**החלטה:** יועצים רואים רק בקשות שלהם, עובדים רואים הכל.

**סיבה:**
- כל יועץ רק צריך לראות את הבקשות שלו
- עובדים צריכים views administrative

**יישום:** ב-`DashboardPage.tsx`, פונקציית `useMemo` עם `userRequests` מסננת לפי תפקיד. ה-`DashboardPage` אחראי גם על חישוב ההרשאות והעברת `onAction` + `actionLabel` ל-`RequestCard` — הכרטיס עצמו הוא pure presentational ולא יודע כלום על תפקידים.
...
### 8. היסטוריית הערות (Notes on Status Change)

**החלטה:** כשעובד מחליף סטטוס ומוסיף הערה, ההערה נשמרת כחלק מהבקשה ומוצגת ליועץ בעת העריכה.

**סיבה:**
- יועצים צריכים לדעת מה נדרש מהם (למשל: "בקש הבהרה" + הסבר מה חסר)
- חוסך תקשורת חיצונית (מייל, טלפון) בין עובד ליועץ
- שומר תיעוד מלא של ההחלטות שנלקחו על הבקשה

**יישום:** מודל `RequestNote` ב-`mockRequests.ts`, שמירה ב-`handleUpdateStatus` ב-`RequestFormPage.tsx`, הצגה בפאנל "היסטוריית הערות" ב-`RequestForm.tsx`.

---

### 9. ניהול הרשאות מרוכז (Centralized Permissions)
**החלטה:** כל לוגיקת ההרשאות מרוכזת בקובץ אחד — `utils/permissions.ts`.

**סיבה:**
- מניעת כפילויות: לפני הריפקטור, strings כמו `'יועץ'` ו-`EDITABLE_STATUSES` היו מפוזרים ב-4 קבצים שונים
- שינוי עתידי (למשל תפקיד חדש) ידרוש עדכון במקום אחד בלבד
- קומפוננטות presentational (כמו `RequestCard`) לא צריכות לדעת כלום על הרשאות

**מה מוגדר שם:**
- `ROLES` — קבועי תפקידים (`CONSULTANT` / `EMPLOYEE`)
- `EDITABLE_STATUSES` — סטטוסים שמאפשרים עריכה ליועץ
- `consultantCanEdit()` — בדיקת הרשאה ליועץ (בעלות + סטטוס)
- `employeeCanHandle()` — תמיד `true` לעובד
- `canOpenRequest()` — entry point לדשבורד
- `getActionLabel()` — תווית כפתור לפי תפקיד: **"ערוך בקשה"** ליועץ, **"טיפול בבקשה"** לעובד

**יישום:** `DashboardPage.tsx` קורא ל-`canOpenRequest()` ו-`getActionLabel()` ומעביר אותם כ-props ל-`RequestCard`. הכרטיס עצמו הוא pure presentational — ללא `useUser()` וללא `useNavigate()`.

---

### 10. ניתוב מסתור (Protected Routes)
